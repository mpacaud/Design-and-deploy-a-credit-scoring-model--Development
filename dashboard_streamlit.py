...
with col1:
    selected_customer_id = st.selectbox("Customer ID selection", l_customer_id) # For a default value: index=<default_value_idx>
    
    # For convinience if the user select None the empty variable is filled with -1 instead.
    if selected_customer_id == 'Global':
...
with col2:
    ...
    if selected_customer_id != 0:
        ...
        description = "This figure shows in which measure the values of a feature influence the feature in positive or negative way\
                       ...
                       within each feature."
        st.markdown("<h4 style='text-align: center; color: black;'> Beeswarm plot </h4>", unsafe_allow_html=True, help=description)
